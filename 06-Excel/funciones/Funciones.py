...
class Funciones():
    ...
    def Existe(self, tipo, selector,tiempo):
        if(tipo=="xpath"):
            try:
                val = WebDriverWait(self.driver, tiempo).until(EC.visibility_of_element_located((By.XPATH, selector)))
                val = self.driver.execute_script("arguments[0].scrollIntoView();", val)
            ...
            try:
                val = WebDriverWait(self.driver, tiempo).until(EC.visibility_of_element_located((By.ID, selector)))
                val = self.driver.execute_script("arguments[0].scrollIntoView();", val)
